...
def plotting(data,ticker,return_window,std_dev,x,y,mean_Return,std_dev_Return,anual_vol,rsi_window,short_window,long_window, low_volatility ,high_volatility):
   # Get the latest signals (last non-None)
    ...
    if data['ADX'].iloc[-1] > 25 :
        print(f"Current ADX has a value of {data['ADX'].iloc[-1]:.2f} suggesting a strong trending market so use moving averages")
        print(f"the last signal for moving averegs is to {latest_signal_MA}")
    else:
        print(f"Current ADX has a value of {data['ADX'].iloc[-1]:.2f} suggesting a choppy market so use RSI")
        print(f"the last signal for RSI is to {latest_signal_RSI}")
    ...
    print(f"Mean of {ticker} Returns: {mean_Return:.2f} %")
    print(f"overall Standard Deviation(volatitly) of {ticker} Retruns: {std_dev_Return:.2f}% over rolling window of {return_window} days ")
    print(f'the anual volatitly fo {ticker} is {anual_vol:.2f}%')
...
def launch_analysis():
    ...
    plotting(df, ticker, return_window, std, x, y, mean_return, std, anual_vol, rsi_window, short_window, long_window, low_vol, high_vol)
...
start_date_var = tk.StringVar(value="2022-01-01")
